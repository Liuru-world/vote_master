...
public interface ActivitiMapper {
    ...
    // 新增表单
    @Insert("<script> insert into formdata (PROC_DEF_ID_,PROC_INST_ID_,FORM_KEY_,Control_ID_,Control_VALUE_,Control_LABEL_)"
            + "values"
            ...
            + "#{formData.Control_LABEL_,jdbcType=VARCHAR})"
            + "</foreach>"
            + "</script>")
    ...
    @Select("select count(ID_)" +
            "from ACT_RU_EXECUTION where NAME_ like concat(#{instanceName,jdbcType=VARCHAR},'%')")
    int countInstanceName(@Param("instanceName")String instanceName);
